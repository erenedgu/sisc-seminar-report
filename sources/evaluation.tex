...
To address the reliance on execution time as a proxy for energy consumption (as highlighted in Section~\ref{sec:motivation}), this study evaluates whether time-to-solution reliably predicts process-attributed energy draw across varying mesh resolutions. Specifically, this experiment addresses Sub-question 2 by assessing the linearity of the time-energy relationship under deterministic ($\sigma = 0.0\,\text{m}$) execution conditions.
...
For each ensemble, the mean power draw is defined as the slope of a linear regression line fit to the cumulative energy curve. Calculating the mean ($\mu$) of this power draw across the ensemble establishes the true hardware baseline for each spatial resolution tier.
...
Each of the five ensembles consists of $100$ equiprobable realizations. This sample size was selected as a computationally feasible benchmark to capture primary distributional trends while constraining total execution time and energy expenditure across the multiple noise tiers.
...
\begin{itemize}
    ...
    \item \textbf{Coefficient of Variation (CV):} Normalizes the absolute error relative to the mean depth ($\text{CV} = \frac{\sigma}{\mu}$), providing a dimensionless metric to evaluate relative uncertainty across different perturbation tiers.
\end{itemize}
...
As expected, increasing the topographical noise ($\sigma_{\text{input}}$) directly degrades the certainty of the physical simulation. At high-quality sensor resolutions ($\sigma = 0.05\,\text{m}$ and $0.1\,\text{m}$), the model output remains relatively stable, showing a CV below $15\%$. However, as the DEM accuracy drops to levels typical of dense vegetation or older legacy datasets ($\sigma \ge 0.5\,\text{m}$), relative uncertainty increases sharply, with the CV exceeding $50\%$. At $\sigma = 1.0\,\text{m}$, the standard deviation of the water depth reaches $\pm 0.817\,\text{m}$, resulting in a CV of $74.21\%$. For this specific coordinate, a variance this large suggests that localized flood predictions may become unreliable for risk assessment, as the margin of error approaches the predicted flood depth itself.

While the increase of the standard deviation was anticipated, the behavior of the expected water depth reveals a distinct non-linear effect. Although the injected Gaussian noise was strictly zero-mean ($\mu = 0$), the ensemble mean water depth does not remain constant across ensembles. Instead, the mean water depth rises systematically from $0.739\,\text{m}$ at the lowest noise tier to $1.101\,\text{m}$ at the highest.
...
This metric is selected to quantify the dispersion of total energy consumption across ensemble realizations and capture upper-bound computational demands. Evaluating the spread between the 5th and 95th percentiles provides a measure of resource variability while remaining robust against extreme statistical outliers.
...
By comparing the median and mean, the data reveals a distinction between the solver's typical operational cost and its upper-bound tail behavior. Across all ensembles, the mean consistently exceeds the median. This right-skewed distribution indicates that while the baseline energy draw remains stable under typical realizations, steep localized gradients trigger adaptive timestepping, producing a tail of higher-energy runs that elevates the mean. As input noise increases, the overall energy cost trends upward due to these increased numerical stability constraints.
